...
const Billing = () => {
  ...
  const handleItemChange = async (index, field, value) => {
    ...
    if (field === "id") {
      ...
      if (value.trim()) {
        try {
          const res = await fetch(`https://backend-8kv3.onrender.com/api/medicine/${value}`);
          if (res.ok) {
  ...
  const submitInvoice = async () => {
    ...
    try {
      ...
      const stockRes = await fetch("https://backend-8kv3.onrender.com/api/medicine/reduce-stock", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
